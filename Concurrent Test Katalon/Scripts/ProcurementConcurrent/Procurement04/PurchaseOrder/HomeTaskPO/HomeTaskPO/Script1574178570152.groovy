...
if (WebUI.verifyElementNotPresent(findTestObject('Object Repository/Procurement/PurchaseOrder/HomeTaskPO/TableBodyTaskPo'), 
    GlobalVariable.wait_short_time, FailureHandling.OPTIONAL)) {
    'Prevent Panel Task jika belum terbuka'
    WebUI.waitForElementClickable(findTestObject('Object Repository/Procurement/PurchaseOrder/HomeTaskPO/OpenTaskPO'), GlobalVariable.wait_js_long_time)

    CustomKeywords.'customKeywords.CustomKeywordJS.clickUsingJS'(findTestObject('Object Repository/Procurement/PurchaseOrder/HomeTaskPO/OpenTaskPO'), 
        GlobalVariable.wait_js_long_time)
}
...
'To locate table'
WebElement Table = driver.findElement(By.xpath('//*[@id="frmMain:dtPurchaseOrder:tb"]'))
...
for (int i = 0; i < Rows.size(); i++) {
    ...
    try {
        println('variable screen: ' + Cols.get(0).getText().substring(6))

        println('variable csv: ' + PRno.substring(11))

        String PONumber = Cols.get(0).getText().substring(6)

        if (PONumber.equalsIgnoreCase(PRno.substring(11))) {
            List<WebElement> action = Rows.get(i).findElements(By.tagName('td'))

            action.get(2).findElement(By.tagName('a')).click()
